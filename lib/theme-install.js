...
if (process.argv.length > 2) {
  // Verify if the passed argument is a git repo link.
  if (repoLink.indexOf('.git') < 0)
    return console.error('Error: No .git extension found. The passed argument is not a git repository link');
  ...
  urlCheck(repoLink, exist => {
    if (!exist) return console.error('Error: the passed link does not exist');
