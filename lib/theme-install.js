    ...
    dirCheck(endPath, (err, exist) => {
      ...
      if (exist) {
        ...
        prompt(rl, 'Do you want to replace the directory? y/n ', (err, replace) => {
          ...
          del([endPath]).then(installTheme(endPath, themeName));
          rl.close();
        });
      } else {
        installTheme(endPath, themeName);
      }
    });
  });
} else {
...
function installTheme(endPath, themeName) {
    ...
    isSupported(endPath, (err, supported) => {
      ...
      if (supported) {
        ...
        prompt(rl, '\nDo you want to set this theme as used theme in the config.js file? y/n ', (err, set) => {
          ...
          if (set) {
            // Change theme in config.js to new theme name
            fs.readFile(path.resolve('config.js'), 'utf-8', (err, data) => {
              if (err) throw err;
              const newData = data.replace(/theme: '.*?'/, `theme: '${themeName}'`);
